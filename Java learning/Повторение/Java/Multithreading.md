 ...
 **ThreadPoolExecutor**
Класс `ThreadPoolExecutor` — реализация `ExecutorService`, которая управляет пулом потоков. Позволяет настраивать количество ядерных и максимальных потоков, время простоя и очередь задач. Гибкость конфигурации делает его подходящим для большинства сценариев многопоточности. Используется для оптимизации использования ресурсов при выполнении асинхронных задач.
